...
dggridR.rmd citation info Next time too: Is there some reference about
the method you can add in the Description field in the form Authors
(year) \<doi:…..\>? Update Vignette to use new GEO_to_SEQNUM commands
...
library(“sf”) library(“rgeos”) sf_poly \<- as(xy.sp, “sf”)
sf::st_crs(sf_poly) \<- 4326 sf_poly\$id \<- c(1,2) ggplot(sf_poly) +
...
[http://r-pkgs.had.co.nz/vignettes.html](http://r-pkgs.had.co.nz/vignettes.md)
...
dgpentsingrid \<- function(grid){ which(unlist(lapply(1:length(grid),
function(i) dgispent(grid,i)))) }

dghexesingrid \<- function(grid){ which(unlist(lapply(1:length(grid),
function(i) dgishex(grid,i)))) }
...
Hex: Clarence Jeanneane Brent Facebook StackOverflow Ron Moen Lee
Frelich eeb-r list ergstudents list

Do you remember when spatial analysis was great?

That was probably before you learned how difficult it is to put a grid
of equally-sized cells on a sphere. Before you felt the terror of
projections and the horror of statistical corrections.

Fortunately, I’ve made spatial analysis great again with an R package
that does divide spheres into equally-sized hexagonal cells (some
technical caveats to this are discussed in the package).

I expect this to come out on CRAN soon, but you can find it at the link
...
library(dggridR) library(ggplot2)

coasts \<- readOGR(dsn=“/z/GSHHS_shp/l”, layer=“GSHHS_l_L1”) coasts \<-
coasts %\>% fortify()
